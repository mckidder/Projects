...
// create a temporary test board using MKState class
...
class MKnode
{
	...
	int xPos;
	int yPos;
	// total distance already travelled to reach the MKnode
	int level;
	// priority=level+heuristic
	int priority;
public:
	MKnode(int xp, int yp, int d, int p)
	{
	...
	{
		level += (player1::dir == 8 ? (i % 2 == 0 ? 10 : 14) : 10);
	}
...
bool operator<(const MKnode & a, const MKnode & b)
{
...
public:
	...
	void rand_gen(Move &move, board game_board, MKplayerstats &ps, MKState &state);
	bool isoverP(Move move, board game_board, MKplayerstats &ps, MKState &state);
};
...
Move Player1::get_move(unsigned short p, board game_board)
{
	...
	if (gettype()) {
		...
		// A-star algorithm.
		static priority_queue<MKnode> pq[2]; // list of open (not-yet-tried) nodes
		...
		// reset the MKnode maps
		for (y = 0; y < player1::m; y++)
		{
			for (x = 0; x < player1::n; x++)
			{
				player1::closed_nodes_map[x][y] = 0;
				player1::open_nodes_map[x][y] = 0;
		...
		for (int tot_stone = 0; tot_stone < 4; tot_stone++) {
			// create the start MKnode and push into list of open nodes
			move.src_y = yStart = ps.getplayerloc(iter);
			move.src_x = xStart = ps.getplayerloc(iter);
			n0 = new MKnode(xStart, yStart, 0, 0);
			n0->updatePriority(game_board);
			pq[pqi].push(*n0);
			player1::open_nodes_map[x][y] = n0->getPriority(); // mark it on the open nodes map

															   // A* search
			while (!pq[pqi].empty())
			{
				// get the current MKnode w/ the highest priority
				// from the list of open nodes
				n0 = new MKnode(pq[pqi].top().getxPos(), pq[pqi].top().getyPos(),
					pq[pqi].top().getLevel(), pq[pqi].top().getPriority());

				x = n0->getxPos(); y = n0->getyPos();

				pq[pqi].pop(); // remove the MKnode from the open list
				player1::open_nodes_map[x][y] = 0;
				// mark it on the closed nodes map
				player1::closed_nodes_map[x][y] = 1;
				...
				for (i = 0; i < player1::dir; i++)
				{
					xdx = x + player1::dx[i]; ydy = y + player1::dy[i];

					if (!(xdx<0 || xdx>player1::n - 1 || ydy<0 || ydy>player1::m - 1 || player1::map[xdx][ydy] == 1
						|| player1::closed_nodes_map[xdx][ydy] == 1))
					{
						// generate a child MKnode
						m0 = new MKnode(xdx, ydy, n0->getLevel(),
							n0->getPriority());
						m0->nextLevel(i);
						m0->updatePriority(game_board);

						// if it is not in the open list then add into that
						if (player1::open_nodes_map[xdx][ydy] == 0)
						{
							player1::open_nodes_map[xdx][ydy] = m0->getPriority();
							pq[pqi].push(*m0);
							// mark its parent MKnode direction
							player1::dir_map[xdx][ydy] = (i + player1::dir / 2) % player1::dir;
						}
						else if (player1::open_nodes_map[xdx][ydy] > m0->getPriority())
						{
							...
							// except the MKnode to be replaced will be ignored
							// and the new MKnode will be pushed in instead
							...
							pq[pqi].pop(); // remove the wanted MKnode
							...
							pqi = 1 - pqi;
							pq[pqi].push(*m0); // add the better MKnode instead
						}
...
bool Player1::isoverP(Move move, board game_board, MKplayerstats &ps, MKState &state)
{
	unsigned short x, y;
	bool gameover = false;
	board test_board;
...
void Player1::rand_gen(Move &move, board game_board, MKplayerstats &ps, MKState &state) {
	srand(time(NULL));
	bool check = false;
	int iter = 0;
	do {
